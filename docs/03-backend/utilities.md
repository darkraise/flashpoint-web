...
This document describes the utility functions and middleware used throughout the
backend service.

## Overview

The backend includes several reusable utilities that eliminate code duplication
...
router.get('/games', async (req, res) => {
  ...
  // Query database
  const games = DatabaseService.all('SELECT * FROM game LIMIT ? OFFSET ?', [
    limit,
    offset,
  ]);
...
**Last Updated**: 2026-01-27 **Review Status**: Current
