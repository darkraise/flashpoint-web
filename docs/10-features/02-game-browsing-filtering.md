...
Game browsing and filtering provides a comprehensive interface for discovering
and exploring games from the Flashpoint Archive. Users can search, filter by
multiple criteria, sort results, and navigate through paginated collections with
...
```tsx
<BrowseView platformFilter="Flash" showPlatformFilter={false} />
```
