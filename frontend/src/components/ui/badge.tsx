...
const badgeVariants = cva(
  "inline-flex items-center rounded px-2 py-1 text-xs font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2",
  {
    variants: {
      variant: {
        default:
...
function Badge({ className, variant, children, ...props }: BadgeProps) {
  ...
  const platformClass = variant === 'platform' && typeof children === 'string'
    ? getPlatformColor(children)
    : '';
