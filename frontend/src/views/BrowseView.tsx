...
export function BrowseView() {
  ...
  // Determine platform based on route path
  const routePlatform = useMemo(() => {
    if (location.pathname === "/flash-games") return "Flash";
    if (location.pathname === "/html5-games") return "HTML5";
    return undefined;
  }, [location.pathname]);
  ...
  const filters = {
    ...
    platform: searchParams.get("platform") || routePlatform || undefined,
  ...
  const pageTitle = useMemo(() => {
    ...
    if (
      ...
      return `${filters.platform} Games`;
    }

    // Platform filter with other filters
    if (filters.platform) {
      parts.push(`${filters.platform} Games`);
    }
    ...
    // Default title
    return parts.length > 0 ? parts.join(" ") : "Browse Games";
  }, [
    filters.search,
    filters.platform,
    filters.tags,
